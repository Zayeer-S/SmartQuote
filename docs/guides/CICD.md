...
**Secrets vs config:** Sensitive values (AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY) go in GitHub Secrets. Non-sensitive CI configuration (ports, feature flags, log levels) goes in `.env.test` at the repo root. If you add a new secret, add a placeholder to `.env.example` with a comment explaining what it is.
